...
zcat $NormExpression | tail -n+2 | awk -v i=$NR 'NR > (i-1)*100 && NR <= i*100' |  while read PARAM; do
    ...
    Rscript $FUSION/FUSION.compute_weights.R --bfile $OUT --tmp $OUT.tmp --out $FINAL_OUT --verbose 0 --save_hsq --PATH_gcta $GCTA --PATH_gemma $GEMMA --models blup,lasso,top1,enet,bslmm --covar $COVAR  --hsq_p 0.05 --crossval 5 || continue # used bslmm model, but slowly
